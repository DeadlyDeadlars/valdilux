  ...
  if (status === 'ok') return (
    <div style={{ paddingTop: '5rem', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center', padding: '3rem' }}>
        ...
        <h1 className="serif" style={{ color: '#f0ebe3', fontSize: '2.5rem', fontWeight: 300, marginBottom: '1rem' }}>Спасибо за заказ!</h1>
        <p style={{ color: '#6a6058', fontSize: '0.85rem', marginBottom: '2rem' }}>Мы свяжемся с вами в ближайшее время</p>
    ...
    <div style={{ paddingTop: '5rem', minHeight: '100vh' }}>
      <div style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(201,169,110,0.08)', padding: '4rem 1.5rem 3rem' }}>
        <div style={{ maxWidth: "64rem", margin: "0 auto", padding: "0 1.5rem" }}>
          <h1 className="serif" style={{ color: '#f0ebe3', fontSize: 'clamp(2rem, 4vw, 3rem)', fontWeight: 300 }}>Корзина</h1>
          {count > 0 && <p style={{ color: '#6a6058', fontSize: '0.75rem', marginTop: '0.5rem' }}>{count} товаров</p>}
        </div>
      </div>
      ...
      <div style={{ maxWidth: "64rem", margin: "0 auto", padding: "3rem 1.5rem" }}>
        {items.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '5rem 0' }}>
            <p style={{ color: '#4a4540', fontSize: '0.85rem', marginBottom: '2rem' }}>Корзина пуста</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <div style={{ gridColumn: 'span 2' }}>
              {items.map(({ product, quantity }) => (
                <div key={product.id} style={{ display: 'flex', gap: '1.5rem', padding: '1.5rem 0', borderBottom: '1px solid rgba(201,169,110,0.08)', alignItems: 'center' }}>
                  <div style={{ width: 80, height: 80, background: '#141414', flexShrink: 0, overflow: 'hidden' }}>
                    {product.images?.[0] && <img src={`${API_BASE}${product.images[0]}`} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div className="serif" style={{ color: '#c8bfb0', fontSize: '1rem', fontWeight: 300 }}>{product.name}</div>
                    {product.material && <div style={{ color: '#4a4540', fontSize: '0.65rem', marginTop: '0.25rem' }}>{product.material}</div>}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <span style={{ color: '#c8bfb0', fontSize: '0.85rem', minWidth: 20, textAlign: 'center' }}>{quantity}</span>
                    <button onClick={() => update(product.id, quantity + 1)} style={{ width: 28, height: 28, background: 'none', border: '1px solid rgba(201,169,110,0.2)', color: '#c9a96e', cursor: 'pointer', fontSize: '1rem' }}>+</button>
                  </div>
                  <button onClick={() => remove(product.id)} style={{ background: 'none', border: 'none', color: '#4a4540', cursor: 'pointer', fontSize: '1.2rem', padding: '0 0.5rem' }}>×</button>
                </div>
              ))}
            </div>

            <div>
              <div style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem', position: 'sticky', top: '5rem' }}>
                <div className="section-label mb-4">Итого</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                  <span style={{ color: '#6a6058', fontSize: '0.75rem' }}>Сумма</span>
                  <span style={{ color: '#c8bfb0', fontSize: '0.85rem' }}>{total.toLocaleString('ru-RU')} ₽</span>
                </div>
                {discount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                    <span style={{ color: '#6a6058', fontSize: '0.75rem' }}>Скидка</span>
                    <span style={{ color: '#6a8060', fontSize: '0.85rem' }}>−{discount.toLocaleString('ru-RU')} ₽</span>
                  </div>
                )}
                <div style={{ borderTop: '1px solid rgba(201,169,110,0.08)', paddingTop: '0.75rem', marginBottom: '1.5rem' }}>
                ...
                {!checkout ? (
                  <>
                    ...
                    <button onClick={() => setOneClick(true)} style={{ width: '100%', padding: '0.75rem', background: 'none', border: '1px solid rgba(201,169,110,0.2)', color: '#6a6058', fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase', cursor: 'pointer', marginTop: '0.25rem' }}>
                      Купить в 1 клик
                    </button>
                  ...
                  <form onSubmit={submitOrder} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#6a6058', fontSize: '0.7rem', cursor: 'pointer' }}>
                    ...
                    <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: '0.5rem' }}>Доставка</div>
                    ...
                    <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: '0.5rem' }}>Оплата</div>
