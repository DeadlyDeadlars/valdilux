    ...
    return (
      <div style={{ paddingTop: '5rem', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', padding: '3rem' }}>
          <p style={{ color: '#4a4540', fontSize: '0.85rem', marginBottom: '2rem' }}>Нет товаров для сравнения</p>
    ...
    <div style={{ paddingTop: '5rem', minHeight: '100vh' }}>
      <div style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(201,169,110,0.08)', padding: '3rem 1.5rem 2rem' }}>
        <div style={{ maxWidth: '80rem', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <h1 className="serif" style={{ color: '#f0ebe3', fontSize: 'clamp(1.8rem, 3vw, 2.5rem)', fontWeight: 300 }}>Сравнение товаров</h1>
      ...
      <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '3rem 1.5rem', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ padding: '1rem', textAlign: 'left', borderBottom: '1px solid rgba(201,169,110,0.08)' }}></th>
              {items.map(item => (
                <th key={item.id} style={{ padding: '1rem', borderBottom: '1px solid rgba(201,169,110,0.08)', minWidth: 200 }}>
                  <div style={{ position: 'relative' }}>
                    ...
                    <Link href={`/catalog/${item.slug}`} style={{ textDecoration: 'none' }}>
                      ...
                      <div className="serif" style={{ color: '#c8bfb0', fontSize: '1rem', fontWeight: 300 }}>{item.name}</div>
                    </Link>
                  </div>
                </th>
              ))}
          ...
          <tbody>
            {specs.map(spec => (
              <tr key={spec.key}>
                <td style={{ padding: '1rem', color: '#6a6058', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', borderBottom: '1px solid rgba(201,169,110,0.08)' }}>
                  {spec.label}
                </td>
                {items.map(item => (
                  <td key={item.id} style={{ padding: '1rem', color: '#c8bfb0', fontSize: '0.85rem', borderBottom: '1px solid rgba(201,169,110,0.08)' }}>
