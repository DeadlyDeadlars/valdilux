      ...
      <div style={{ maxWidth: "56rem", margin: "0 auto", padding: "5rem 1.5rem" }}>
        {[
          ...
          },
          {
            ...
            text: 'Мы предлагаем вам изготовление мебели с вниманием к деталям. Наша работа — это не массовое производство, а настоящее мастерство, где каждая деталь проходит тщательную обработку и контроль качества.',
          },
        ].map(({ title, text }) => (
          <div key={title} style={{ marginBottom: '3.5rem', borderLeft: '1px solid rgba(201,169,110,0.2)', paddingLeft: '2rem' }}>
            <h2 className="serif" style={{ color: 'var(--text)', fontSize: '1.6rem', fontWeight: 300, marginBottom: '1rem' }}>{title}</h2>
            <p style={{ color: 'var(--muted)', fontSize: '0.875rem', lineHeight: 1.9 }}>{text}</p>
          </div>
        ))}

        <div style={{ marginBottom: '3.5rem' }}>
          <h2 className="serif" style={{ color: 'var(--text)', fontSize: '1.6rem', fontWeight: 300, marginBottom: '2rem' }}>Возможности изготовления</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { title: 'Индивидуальный дизайн', text: 'Разработка уникальных проектов с учетом ваших пожеланий' },
            ].map(({ title, text }) => (
              <div key={title} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', padding: '1.5rem' }}>
                <h4 style={{ color: '#c9a96e', fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>{title}</h4>
                <p style={{ color: 'var(--muted)', fontSize: '0.75rem', lineHeight: 1.7 }}>{text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', padding: '2rem' }}>
          <h3 className="serif" style={{ color: 'var(--text)', fontSize: '1.2rem', fontWeight: 300, marginBottom: '1rem' }}>ИП Минин Дмитрий Витальевич</h3>
          <div style={{ display: 'flex', gap: '3rem', flexWrap: 'wrap' }}>
            <div>
              <div style={{ color: 'var(--muted2)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>ИНН</div>
              <div style={{ color: '#a09080', fontSize: '0.8rem' }}>661903101020</div>
            </div>
            <div>
              <div style={{ color: 'var(--muted2)', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>ОГРН</div>
