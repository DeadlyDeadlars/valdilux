...
export default function AdminChatPage() {
  ...
  const inp = { background: 'var(--bg3)', border: '1px solid rgba(201,169,110,0.15)', color: 'var(--text2)', padding: '0.75rem 1rem', fontSize: '0.8rem', outline: 'none', fontFamily: 'Inter, sans-serif' };
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg2)', display: 'flex', paddingTop: '4rem' }}>
      {/* Sidebar */}
      <div style={{ width: 240, borderRight: '1px solid var(--border)', padding: '1.5rem 0' }}>
        <div style={{ color: 'var(--muted)', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', padding: '0 1rem', marginBottom: '1rem' }}>
        ...
        {rooms.map(chatId => {
          ...
          return (
            <button key={chatId} onClick={() => selectRoom(chatId)}
              style={{ width: '100%', padding: '0.75rem 1rem', background: activeRoom === chatId ? 'var(--border)' : 'none', border: 'none', borderLeft: activeRoom === chatId ? '2px solid #c9a96e' : '2px solid transparent', cursor: 'pointer', textAlign: 'left' }}
            >
              <div style={{ color: 'var(--text2)', fontSize: '0.7rem' }}>#{chatId.slice(0, 8)}</div>
              {last && <div style={{ color: 'var(--muted2)', fontSize: '0.65rem', marginTop: '0.2rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{last.text}</div>}
            </button>
          );
        })}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {!activeRoom ? (
          ...
          <>
            <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border)', color: 'var(--muted)', fontSize: '0.7rem' }}>
              ...
              {roomMessages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.from === 'manager' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '70%', padding: '0.5rem 0.875rem', fontSize: '0.8rem', lineHeight: 1.5, background: m.from === 'manager' ? 'rgba(201,169,110,0.12)' : '#1a1a1a', color: m.from === 'manager' ? '#c9a96e' : '#c8bfb0', border: `1px solid ${m.from === 'manager' ? 'rgba(201,169,110,0.2)' : 'rgba(255,255,255,0.04)'}` }}>
                    <div style={{ color: 'var(--muted2)', fontSize: '0.55rem', marginBottom: '0.2rem' }}>{m.from === 'manager' ? 'Вы' : 'Посетитель'}</div>
                    {m.text}
                  </div>
                </div>
              ))}
            ...
            <form onSubmit={send} style={{ borderTop: '1px solid var(--border)', display: 'flex', padding: '0.75rem 1.5rem', gap: '0.75rem' }}>
