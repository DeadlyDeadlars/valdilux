  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 className="serif" style={{ color: '#f0ebe3', fontSize: '1.8rem', fontWeight: 300 }}>Товары</h1>
      ...
      {loading ? <div style={{ color: '#4a4540' }}>Загрузка...</div> : (
        <table style={{ width: '100%', borderCollapse: 'collapse', background: '#141414', border: '1px solid rgba(201,169,110,0.08)' }}>
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                <td style={{ ...td, color: '#4a4540' }}>{p.id}</td>
                <td style={td}>{p.name}</td>
                <td style={{ ...td, color: '#6a6058' }}>{p.category?.name}</td>
