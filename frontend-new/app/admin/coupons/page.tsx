...
export default function AdminCouponsPage() {
  ...
  return (
    <div>
      <h1 className="serif" style={{ color: '#f0ebe3', fontSize: '1.8rem', fontWeight: 300, marginBottom: '2rem' }}>Купоны</h1>
      <form onSubmit={save} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem', marginBottom: '2rem', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {coupons.map(c => (
          <div key={c.id} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '0.875rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
              <span style={{ color: '#c9a96e', fontSize: '0.85rem', fontFamily: 'monospace' }}>{c.code}</span>
              <span style={{ color: '#c8bfb0', fontSize: '0.78rem' }}>{c.discount}{c.type === 'percent' ? '%' : ' ₽'}</span>
              {c.minAmount > 0 && <span style={{ color: '#4a4540', fontSize: '0.7rem' }}>от {c.minAmount.toLocaleString('ru-RU')} ₽</span>}
              {c.expiresAt && <span style={{ color: '#4a4540', fontSize: '0.7rem' }}>до {new Date(c.expiresAt).toLocaleDateString('ru-RU')}</span>}
            </div>
