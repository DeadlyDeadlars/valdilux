...
export default function AdminPostsPage() {
  ...
  const inp = { background: '#1a1a1a', border: '1px solid rgba(201,169,110,0.12)', color: '#c8bfb0', padding: '0.6rem 0.75rem', fontSize: '0.78rem', outline: 'none', width: '100%' };

  return (
    <div>
      <h1 className="serif" style={{ color: '#f0ebe3', fontSize: '1.8rem', fontWeight: 300, marginBottom: '2rem' }}>Посты</h1>
      <form onSubmit={save} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem', marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#6a6058', fontSize: '0.75rem' }}>
          <input type="checkbox" checked={form.published} onChange={e => setForm(f => ({ ...f, published: e.target.checked }))} />
          Опубликовать
        </label>
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
          {editing && <button type="button" onClick={() => { setForm(empty); setEditing(null); }} style={{ padding: '0.5rem 1rem', background: 'none', border: '1px solid rgba(201,169,110,0.2)', color: '#6a6058', fontSize: '0.7rem', cursor: 'pointer' }}>Отмена</button>}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        {posts.map(p => (
          <div key={p.id} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '0.875rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <span style={{ color: '#c8bfb0', fontSize: '0.8rem' }}>{p.title}</span>
              <span style={{ color: '#4a4540', fontSize: '0.65rem', marginLeft: '0.75rem' }}>{p.type}</span>
