      ...
      <aside style={{ width: 200, background: '#111', borderRight: '1px solid rgba(201,169,110,0.1)', padding: '2rem 0', flexShrink: 0 }}>
        ...
        {nav.map(n => (
          <Link key={n.href} href={n.href}
            style={{ display: 'block', padding: '0.6rem 1.5rem', color: '#6a6058', fontSize: '0.75rem', textDecoration: 'none' }}
          >
            {n.label}
          </Link>
        ))}
        <form action="/api/admin/logout" method="POST" style={{ marginTop: '2rem', padding: '0 1.5rem' }}>
          <button type="submit" style={{ background: 'none', border: 'none', color: '#4a4540', fontSize: '0.7rem', cursor: 'pointer' }}>
            Выйти
          </button>
        </form>
      </aside>
