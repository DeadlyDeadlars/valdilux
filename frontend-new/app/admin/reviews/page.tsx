...
export default function AdminReviewsPage() {
  ...
  return (
    <div>
      <h1 className="serif" style={{ color: '#f0ebe3', fontSize: '1.8rem', fontWeight: 300, marginBottom: '2rem' }}>Отзывы</h1>
      {loading ? <div style={{ color: '#4a4540' }}>Загрузка...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {reviews.map(r => (
            <div key={r.id} style={{ background: '#141414', border: `1px solid ${r.approved ? 'rgba(106,128,96,0.2)' : 'rgba(201,169,110,0.08)'}`, padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.4rem' }}>
                  <span style={{ color: '#c8bfb0', fontSize: '0.8rem' }}>{r.name}</span>
                  <span style={{ color: '#c9a96e', fontSize: '0.75rem' }}>{'★'.repeat(r.rating)}</span>
                  <span style={{ color: '#4a4540', fontSize: '0.7rem' }}>{r.product?.name}</span>
                </div>
                <p style={{ color: '#6a6058', fontSize: '0.78rem', margin: 0 }}>{r.comment}</p>
              </div>
