...
export default function AccountPage() {
  ...
  return (
    <div style={{ paddingTop: '5rem', minHeight: '100vh' }}>
      <div style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(201,169,110,0.08)', padding: '3rem 1.5rem 2rem' }}>
        <div style={{ maxWidth: '64rem', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 className="serif" style={{ color: '#f0ebe3', fontSize: 'clamp(1.8rem, 3vw, 2.5rem)', fontWeight: 300 }}>Личный кабинет</h1>
            <p style={{ color: '#6a6058', fontSize: '0.75rem', marginTop: '0.25rem' }}>{user.email}</p>
          </div>
      ...
      <div style={{ maxWidth: '64rem', margin: '0 auto', padding: '3rem 1.5rem' }}>
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '2.5rem', borderBottom: '1px solid rgba(201,169,110,0.08)' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '3rem 0', color: '#6a6058', fontSize: '0.8rem' }}>Загрузка...</div>
        ) : tab === 'orders' ? (
          orders.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 0' }}>
              <p style={{ color: '#4a4540', fontSize: '0.85rem', marginBottom: '1.5rem' }}>У вас пока нет заказов</p>
              <Link href="/catalog" className="btn-gold">Перейти в каталог</Link>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              {orders.map(order => (
                <div key={order.id} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <div>
                      <div style={{ color: '#c9a96e', fontSize: '0.65rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Заказ #{order.id}</div>
                      <div style={{ color: '#6a6058', fontSize: '0.7rem', marginTop: '0.25rem' }}>{new Date(order.createdAt).toLocaleDateString('ru-RU')}</div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ color: '#c8bfb0', fontSize: '0.9rem' }}>{order.total.toLocaleString('ru-RU')} ₽</div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {order.items.map(item => (
                      <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', color: '#6a6058', fontSize: '0.75rem' }}>
          ...
          wishlist.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 0' }}>
              <p style={{ color: '#4a4540', fontSize: '0.85rem', marginBottom: '1.5rem' }}>Избранное пусто</p>
              <Link href="/catalog" className="btn-gold">Перейти в каталог</Link>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {wishlist.map(p => (
                <div key={p.id} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1rem', position: 'relative' }}>
                  ...
                  <Link href={`/catalog/${p.slug}`} style={{ textDecoration: 'none' }}>
                    <div className="serif" style={{ color: '#c8bfb0', fontSize: '0.9rem', fontWeight: 300, marginBottom: '0.5rem' }}>{p.name}</div>
          ...
          <div style={{ maxWidth: '28rem' }}>
            ...
            <div style={{ borderTop: '1px solid rgba(201,169,110,0.08)', marginTop: '2.5rem', paddingTop: '2rem' }}>
              <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '1rem' }}>Смена пароля</div>
