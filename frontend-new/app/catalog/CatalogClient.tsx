...
const MATERIALS = ['Дуб', 'Орех', 'Ясень'];
const SORTS = [
    ...
    <div style={{ paddingTop: '5rem' }}>
      ...
      <div style={{ background: '#0a0a0a', borderBottom: '1px solid rgba(201,169,110,0.08)', padding: '4rem 1.5rem 3rem' }}>
        <div style={{ maxWidth: "80rem", margin: "0 auto", padding: "0 1.5rem" }}>
          ...
          <h1 className="serif" style={{ color: '#f0ebe3', fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 300 }}>Каталог мебели</h1>
          <p style={{ color: '#6a6058', fontSize: '0.8rem', marginTop: '0.75rem' }}>Каждое изделие создано из премиальных материалов</p>
        </div>
      </div>
      ...
      <div style={{ maxWidth: "80rem", margin: "0 auto", padding: "3rem 1.5rem" }} className="flex gap-10 flex-col md:flex-row">
        ...
        <aside style={{ minWidth: 200 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            ...
            <button onClick={reset} style={{ color: '#4a4540', fontSize: '0.6rem', letterSpacing: '0.1em', background: 'none', border: 'none', cursor: 'pointer', textTransform: 'uppercase' }}>Сбросить</button>
          </div>

          <div style={{ marginBottom: '2rem' }}>
            <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>Категория</div>
            {categories.map(cat => (
          ...
          <div>
            <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>Материал</div>
            {MATERIALS.map(m => (
          ...
          <div style={{ marginTop: '2rem' }}>
            <div style={{ color: '#6a6058', fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>Цена, ₽</div>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              <input type="number" placeholder="От" value={priceMin} onChange={e => setPriceMin(e.target.value)}
                style={{ width: '50%', background: '#141414', border: '1px solid rgba(201,169,110,0.12)', color: '#c8bfb0', padding: '0.4rem 0.5rem', fontSize: '0.7rem', outline: 'none' }} />
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
            <span style={{ color: '#6a6058', fontSize: '0.75rem' }}>
          ...
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} style={{ background: '#141414', aspectRatio: '3/4', animation: 'pulse 1.5s infinite' }} />
              ))}
            </div>
            ...
            <div style={{ textAlign: 'center', padding: '5rem 0', color: '#4a4540', fontSize: '0.8rem' }}>Товары не найдены</div>
          )}
        </div>
      </div>
    </div>
