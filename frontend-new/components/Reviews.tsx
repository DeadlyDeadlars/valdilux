  ...
  return (
    <div style={{ marginTop: '5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 className="serif" style={{ color: '#f0ebe3', fontSize: '1.5rem', fontWeight: 300 }}>Отзывы</h2>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={submit} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem', marginBottom: '2rem' }}>
          <div style={{ marginBottom: '1rem' }}>
            <label style={{ color: '#6a6058', fontSize: '0.7rem', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: '0.5rem' }}>Оценка</label>
          ...
          {reviews.map(r => (
            <div key={r.id} style={{ background: '#141414', border: '1px solid rgba(201,169,110,0.08)', padding: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                <div>
                  <div style={{ color: '#c8bfb0', fontSize: '0.85rem', fontWeight: 400 }}>{r.name}</div>
              ...
              <p style={{ color: '#6a6058', fontSize: '0.85rem', lineHeight: 1.7 }}>{r.comment}</p>
            </div>
          ))}
        </div>
      )}
